...
export default function VendorAdsIndex({ ads, filters, linkTypeOptions, statusOptions, products, expos, subscriptionInfo, isAdmin = false }: VendorAdsIndexProps) {
    ...
    const exportAds = async () => {
        ...
        try {
            ...
            // Use different export endpoint based on user type
            const exportUrl = isAdmin ? `/ads/export?${params.toString()}` : `/vendor/ads/export?${params.toString()}`;
    ...
    const handleDelete = async (ad: Ad) => {
        setIsDeleting(true);
        try {
            // Use different delete endpoint based on user type
            const deleteUrl = isAdmin ? `/ads/${ad.id}` : `/vendor/ads/${ad.id}`;
    ...
    const confirmToggleStatus = async () => {
        ...
        try {
            // Use different toggle endpoint based on user type
            const toggleUrl = isAdmin ? `/ads/${confirmDialog.ad.id}/toggle-status` : `/vendor/ads/${confirmDialog.ad.id}/toggle-status`;
            await router.post(toggleUrl, {}, {
    ...
    return (
        <AppLayout>
            <Head title={isAdmin ? "All Ads" : "My Ads"} />
            <div className="space-y-8 px-2 py-6 md:px-6">
                <div className="mb-2 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <Heading title={isAdmin ? "All Ads" : "My Ads"} />
                    ...
                    <Alert className={subscriptionInfo.hasSubscription ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}>
                        <AlertDescription>
                            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                <div>
                                    <strong>Current Plan:</strong> {subscriptionInfo.planName}
                                    {subscriptionInfo.hasSubscription && (
                                        <span className="ml-4">
                                            <strong>Ad Limit:</strong> {subscriptionInfo.currentAds} / {subscriptionInfo.adLimit}(
                                            {subscriptionInfo.remainingAds} remaining)
                                        </span>
                                    )}
                                </div>
                                {!subscriptionInfo.hasSubscription && (
                ...
                <div className="flex flex-col justify-end gap-2 sm:flex-row sm:items-center">
                    <div>
                        <Input
                            ...
                            onChange={(e) => {
                                const baseUrl = isAdmin ? '/ads' : '/vendor/ads';
                                router.get(baseUrl, { ...filters, search: e.target.value }, { preserveState: true });
                            }}
                        />
                    </div>
                    <div className="w-40">
                        <Select
                            ...
                            onValueChange={(value) => {
                                const baseUrl = isAdmin ? '/ads' : '/vendor/ads';
                                router.get(baseUrl, { ...filters, status: value }, { preserveState: true });
                            }}
                        >
                            <SelectTrigger>
                                <SelectValue />
                    ...
                    <div className="w-40">
                        <Select
                            ...
                            onValueChange={(value) => {
                                const baseUrl = isAdmin ? '/ads' : '/vendor/ads';
                                router.get(baseUrl, { ...filters, link_type: value }, { preserveState: true });
                            }}
                        >
                            <SelectTrigger>
                                <SelectValue />
                ...
                <Card className="border-border rounded-xl border shadow-md">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-lg font-semibold">
                            {isAdmin ? "All Ads" : "My Ads"} <span className="text-muted-foreground font-normal">({ads.total})</span>
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                            ...
                            <table className="w-full border-separate border-spacing-y-1 text-sm">
                                <thead>
                                    <tr className="bg-muted/60 dark:bg-muted/40 text-muted-foreground">
                                        <th className="rounded-tl-lg p-3 text-left">Banner</th>
                                        <th className="p-3 text-left">Title (EN)</th>
                                        <th className="p-3 text-left">Title (AR)</th>
                                        <th className="p-3 text-left">Link Type</th>
                                        <th className="p-3 text-left">Linked Item</th>
                                        <th className="p-3 text-left">Status</th>
                                        <th className="p-3 text-left">Priority</th>
                                        <th className="p-3 text-left">Date Range</th>
                                        <th className="rounded-tr-lg p-3 text-left">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {ads.data.map((ad) => (
                                        <tr key={ad.id} className="bg-card dark:bg-muted/60 hover:bg-muted/30 border-border rounded-lg border shadow-sm transition">
                                            ...
                                            <td className="p-3 align-middle text-center">
                                                {ad.start_date && ad.end_date ? (
                                                    ...
                                                    <span className="text-muted-foreground text-sm">No date range</span>
                                                )}
                                            </td>
                                            <td className="p-3 align-middle">
                                                <div className="flex items-center gap-2">
                                                    <ActionButtonSuspend
                                                        ...
                                                        btnTitle={ad.status === 'active' ? 'Suspend' : 'Activate'}
                                                    />
                                                    ...
                                                    <ActionButtonDelete btnTitle="Delete" onClick={() => openDeleteDialog(ad)} />
            ...
            {confirmDialog && (
                <Dialog open={confirmDialog.open} onOpenChange={(open) => setConfirmDialog(confirmDialog ? { ...confirmDialog, open } : null)}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>{confirmDialog.action === 'activate' ? 'Activate Ad' : 'Suspend Ad'}</DialogTitle>
                        </DialogHeader>
                        <div className="py-4">
                            <p>
                        ...
                        <DialogFooter>
                            ...
                            >
                                {toggleLoadingId === confirmDialog.ad.id
                                    ? (confirmDialog.action === 'activate' ? 'Activating...' : 'Suspending...')
                                    : (confirmDialog.action === 'activate' ? 'Activate' : 'Suspend')}
                            </Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
                    ...
                    <div className="py-4">
                        ...
                        {selectedAdForDelete && (
                            <p className="mt-2 text-sm text-gray-600">
                                <strong>Ad:</strong> {selectedAdForDelete.title_en}
                            </p>
                        )}
                    </div>
                    <DialogFooter>
